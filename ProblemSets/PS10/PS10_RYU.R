...
# 3-fold cross-validation for the tree model
tree_folds <- vfold_cv(income_train, v = 3)



# Workflow for tree model
tree_wf <- workflow() %>%
...
# Finding the best tree model based on accuracy
top_tree  <- show_best(tree_res, metric = "accuracy")
best_tree <- select_best(tree_res, metric = "accuracy")


# Finalizing the workflow with the best parameters
final_tree_model <- finalize_workflow(tree_wf, best_tree)


# Fitting the final tree model to the full training set and evaluating on the test set
final_tree_results <- last_fit(final_tree_model, income_split) %>%
  collect_metrics()


# Printing the model evaluation results
final_tree_results %>% print()

final_tree_results %>% print(n = 1)
top_tree %>% print(n = 1)

tree_ans <- top_tree %>% slice(1)
tree_ans %<>% left_join(final_tree_results %>% slice(1),by=c(".metric",".estimator")) %>%
...
# YOU FILL IN THE REST
# 3-fold cross-validation for the neural net model
nnet_folds <- vfold_cv(income_train, v = 3)


# Workflow for neural network model
nnet_wf <- workflow() %>%
...
# Finding the best neural network model based on accuracy
...
# Fitting the final neural network model to the full training set and evaluating on the test set
final_nnet_results <- last_fit(final_nnet_model, income_split) %>%
  collect_metrics()

# Printing the model evaluation results
final_nnet_results %>% print()



final_nnet_results %>% print(n = 1)
top_nnet %>% print(n = 1)

nnet_ans <- top_nnet %>% slice(1)
nnet_ans %<>% left_join(final_nnet_results %>% slice(1),by=c(".metric",".estimator")) %>%
...
# YOU FILL IN THE REST
# 3-fold cross-validation for the KNN model
knn_folds <- vfold_cv(income_train, v = 3)

# Workflow for KNN model
knn_wf <- workflow() %>%
...
# Finding the best KNN model based on accuracy
top_knn  <- show_best(knn_res, metric = "accuracy")
best_knn <- select_best(knn_res, metric = "accuracy")


# Finalizing the workflow with the best parameters
final_knn_model <- finalize_workflow(knn_wf, best_knn)

# Fitting the final KNN model to the full training set and evaluating on the test set
final_knn_results <- last_fit(final_knn_model, income_split) %>%
  collect_metrics()

# Printing the model evaluation results
final_knn_results %>% print()


final_knn_results %>% print(n = 1)
top_knn %>% print(n = 1)

knn_ans <- top_knn %>% slice(1)
knn_ans %<>% left_join(final_knn_results %>% slice(1),by=c(".metric",".estimator")) %>%
...
# YOU FILL IN THE REST
# 3-fold cross-validation for the SVM model
...
# I got some error message saying that
# → A | error:   $ operator is invalid for atomic vectors
# There were issues with some computations   A: x1


# Finding the best SVM model based on accuracy
top_svm  <- show_best(svm_res, metric = "accuracy")
best_svm <- select_best(svm_res, metric = "accuracy")


# Finalizing the workflow with the best parameters
final_svm_model <- finalize_workflow(svm_wf, best_svm)

# Fitting the final SVM model to the full training set and evaluating on the test set
final_svm_results <- last_fit(final_svm_model, income_split) %>%
  collect_metrics()

# Printing the model evaluation results
final_svm_results %>% print()



final_svm_results %>% print(n = 1)
top_svm %>% print(n = 1)

svm_ans <- top_svm %>% slice(1)
svm_ans %<>% left_join(final_svm_results %>% slice(1),by=c(".metric",".estimator")) %>%
  mutate(alg = "logit") %>% select(-starts_with(".config"))
